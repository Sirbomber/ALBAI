...
void DisasterCreator::SetCallbackTrigger(char const *callback, int minDelay, int maxDelay)
{
	...
	disCheck = CreateTimeTrigger(1, 0, minDelay, maxDelay, callback);
}

void DisasterCreator::SetCallbackTrigger(char const *callback, int delay)
{
	if (disCheck.IsInitialized())
	{
		disCheck.Destroy();
	...
	disCheck = CreateTimeTrigger(1, 0, delay, callback);
}
