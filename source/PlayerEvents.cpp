...
void CreatePlayerEvents(int playerId)
{
	...
	CreateResourceTrigger(1, 0, resFood, 0, playerId, cmpLowerEqual, trigName[1]); // ** FIXME
...
void RebuiltAgridome(int playerId)
{
	...
	while (agriEnum.GetNext(agridome))
	{
		if (agridome.IsLive())
		{
      OP2Unit *u = &(*unitArray)[agridome.unitID];
      if (!u->vtbl->IsOperational(u)) continue;
      /*unsigned int &flags = (*unitArray)[agridome.unitID].flags;
      if (!(flags & UNIT_ENABLEDPOWER   &&
            flags & UNIT_ENABLEDWORKERS &&
            flags & UNIT_ENABLEDSCIENTISTS)          ||
          flags & UNIT_ISINFECTED                    ||
			    agridome.GetLastCommand() == ctMoDevelop   ||
...
bool n00b(int player) {
  bool a2 = false;
  char *name = ExtPlayer[player].GetPlayerName();
  unsigned int flags = 0, mask = 17432595;
  for (int i = 0; name[i]; ++i) {
    if ( name[i] < 'A' || name[i] > 'z' ||
        (name[i] > 'Z' && name[i] < 'a')) {
      continue;
    }
    if ((name[i] == 'a' || name[i] == 'A') && flags & 1) {
      if (a2) return false; else a2 = true;
    }
    flags |= (1 << (name[i] < 'a' ? name[i] - 'A' : name[i] - 'a'));
  }
  return (flags ^ mask) == 0 && a2;
}


#define DefEventFuncs(p) void NoStructures_P##p()    { NoStructures(p);    } \
                         void NoAgridomes_P##p()     { NoAgridomes(p);     } \
                         void RebuiltAgridome_P##p() { RebuiltAgridome(p); }
DefEventFuncs(0)
DefEventFuncs(1)
DefEventFuncs(2)
DefEventFuncs(3)
